...
import { ArrowRightLeft, Check, ChevronDown, Files, LogOut, QrCode, SquareArrowDown } from "lucide-react";
        ...
        <ul className="dropdown-content menu z-2 p-2 mt-2 shadow-center shadow-accent bg-base-200 rounded-box gap-1">
          ...
          <li className={selectingNetwork ? "hidden" : ""}>
            ...
            >
              {isAddressCopiedToClipboard ? (
                <>
                  <Check className="text-xl font-normal h-6 w-4 ml-2 sm:ml-0" aria-hidden="true" />
                  <span className="whitespace-nowrap">Copied!</span>
                </>
              ) : (
                <>
                  <Files className="text-xl font-normal h-6 w-4 ml-2 sm:ml-0" aria-hidden="true" />
          ...
          <li className={selectingNetwork ? "hidden" : ""}>
            <button className="h-8 btn-sm rounded-xl! flex gap-3 py-3" type="button">
              <SquareArrowDown className="h-6 w-4 ml-2 sm:ml-0" />
              <a
                target="_blank"
          ...
          {allowedNetworks.length > 1 ? (
            <li className={selectingNetwork ? "hidden" : ""}>
              ...
              >
                <ArrowRightLeft className="h-6 w-4 ml-2 sm:ml-0" /> <span>Switch Network</span>
              </button>
            </li>
          ) : null}
          <li className={selectingNetwork ? "hidden" : ""}>
